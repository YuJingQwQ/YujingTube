<template>
  <div id="article-div">
      ...
      <div v-for="article in page.records" :key="article.id">
        ...
        <div class="article-operation">
          ...
          <el-button
            ...
            @click="off(article.id)"
            style="background-color: #181818; border-color: red; color: red"
            type="primary"
            >下架视频</el-button
          >
          ...
          <el-button
            ...
            @click="pass(article.id)"
            style="background-color: #ede908; border-color: red; color: red"
            type="primary"
            >重新上架</el-button
          >
        </div>
    ...
    <div class="article-footer-scope">
  ...
  methods: {
    ...
    pass(articleId) {
      this.$confirm("确定要将此视频通过审核吗?", "", {
        ...
        .then(() => {
          this.$request.admin
            .get("api/pass/article/of/authors/" + articleId)
            .then(({ data }) => {
              if (data.code == 200) {
    ...
    off(articleId) {
      this.$confirm("确定要将此视频通下架吗?", "", {
        ...
        .then(() => {
          this.$request.admin
            .get("api/off/article/of/authors/" + articleId)
            .then(({ data }) => {
              if (data.code == 200) {
                this.$message.success("请求成功");
                this.initArticlePage();
